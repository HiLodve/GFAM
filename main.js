...
const modules = [
  ...
  { id: 'factory', aliases: ['factory', 'build', '制造', '制造自动化', '人形制造', '装备制造', 'dollfactory', 'equipfactory'], title: '制造自动化设置', menuTitle: '制造自动化设置（人形/装备后台制造）', file: 'gfam_factory_config.py', hiddenOnServers: [] },
...
function getFactoryState() { return readJson(path.join(ROOT, '.gfam_factory_state.json'), {}); }
function getDollFactoryEnabled() { const st = getFactoryState(); return st.doll_enabled === true; }
function getEquipFactoryEnabled() { const st = getFactoryState(); return st.equip_enabled === true; }
function factoryStatusText() {
  const st = getFactoryState();
  const doll = st.doll_enabled === true ? ('人形开启/' + (st.doll_formula || '未选公式')) : '人形关闭';
  const equip = st.equip_enabled === true ? ('装备开启/' + (st.equip_formula || '未选公式')) : '装备关闭';
  return doll + '，' + equip;
}
...
function printMenu() {
  ...
  console.log(`  factory      : 制造自动化：${factoryStatusText()}`);
  ...
  console.log('提示：在 factory 中开启人形/装备自动制造后，功能模块运行期间会后台执行制造循环。');
  console.log('================================================\n');
}
...
function runModule(item) {
  ...
  if (getDollFactoryEnabled()) {
    lines.push('set "GFAM_DOLL_FACTORY_AUTO_ENABLED=1"');
  } else {
    lines.push('set "GFAM_DOLL_FACTORY_AUTO_ENABLED=0"');
  }
  if (getEquipFactoryEnabled()) {
    lines.push('set "GFAM_EQUIP_FACTORY_AUTO_ENABLED=1"');
  } else {
    lines.push('set "GFAM_EQUIP_FACTORY_AUTO_ENABLED=0"');
  }
  if (auth) {
