...
_epa_index_mgr = None  # IndexCacheManager 实例，延迟初始化
...
def apply_epa_settings_from_file():
    """从 .gfam_epa_settings.json 加载 EPA 系列设置并应用到 CONFIG。"""
    ...
    if target_module and target_module != "epa":
        return False

    # Mode: team / single
    mode = str(settings.get("mode", "")).strip()
    if mode in ("team", "single"):
        CONFIG["MODE_NAME"] = mode
        CONFIG["SINGLE_GUN_MODE"] = (mode == "single")
        CONFIG["MODE_SELECTED_EARLY"] = True
        applied.append("mode=" + mode)
    ...
    try:
        ...
        if 1 <= tc <= 10:
            ...
            CONFIG["AUTO_CAPTURE_EXPECTED_COUNT"] = tc if mode == "team" else 1
    ...
    # Equip auto-lock
    if "equip_auto_lock" in settings:
        CONFIG["AUTO_LOCK_TARGET_EQUIP"] = bool(settings["equip_auto_lock"])
        applied.append("equip_auto_lock=%s" % CONFIG["AUTO_LOCK_TARGET_EQUIP"])

    # Difficulty / stage / target
    diff = str(settings.get("difficulty", "")).strip()
    stage = str(settings.get("stage", "")).strip()
    target = str(settings.get("target", "")).strip()
    if diff in ("普通", "紧急", "夜战"):
        CONFIG["SELECTED_DIFFICULTY"] = diff
        applied.append("difficulty=" + diff)
    if stage:
        CONFIG["SELECTED_STAGE"] = stage
        applied.append("stage=" + stage)
    if target:
        CONFIG["SELECTED_TARGET"] = target
        applied.append("target=" + target)

    # Advanced: equip auto retire
    if "enable_equip_retire" in settings:
        CONFIG["ENABLE_EQUIP_AUTO_RETIRE"] = bool(settings["enable_equip_retire"])
        applied.append("enable_equip_retire=%s" % CONFIG["ENABLE_EQUIP_AUTO_RETIRE"])

    try:
        rank = int(settings.get("equip_retire_max_rank", 0))
        if 2 <= rank <= 5:
            CONFIG["EQUIP_AUTO_RETIRE_MAX_RANK"] = rank
            applied.append("equip_retire_max_rank=%d" % rank)
    except Exception:
        pass
    ...
    if applied:
        print("[*] 已从 EPA 设置文件加载 %d 项配置：" % len(applied))
        for item in applied:
            print("    %s" % item)
    return bool(applied)
...
def build_team_configs_from_index(payload: dict, require_fairy: bool = True):
    ...
    for team_id in sorted(team_map.keys()):
        ...
        if require_fairy:
            ok = bool(team_cfg["fairy_id"] and team_cfg["guns"])
        else:
            ok = bool(team_cfg["guns"])
        if ok:
...
def try_update_auto_capture_from_index_payload(payload: dict) -> bool:
    ...
    is_team_mode = CONFIG.get("MODE_NAME") == "team"
    teams = build_team_configs_from_index(payload, require_fairy=is_team_mode)
    if not teams:
        return False
    ...
    if is_team_mode:
        ...
        # single 模式必须使用梯队1；如果梯队1未配妖精或人形则报错。
        target_team = None
        for t in teams:
            if t["team_id"] == 1:
                target_team = t
        ...
        if target_team is None:
            print("[!] single 模式要求梯队1必须编入至少 1 名人形。")
            print("[!] 当前 Index/index 解析结果中梯队1没有人形。")
            print("[!] 请前往游戏编队界面，将梯队1配置为单人编队（至少 1 名人形），再重新输入 -a。")
            available = ", ".join(str(t["team_id"]) for t in teams) if teams else "无"
            print("[!] 当前有效梯队：%s" % available)
            return False
        CAPTURED_TEAM_CONFIGS.append({
            "team_id": target_team["team_id"],
            "fairy_id": target_team["fairy_id"],
            "fairy": copy.deepcopy(target_team.get("fairy")),
            "guns": copy.deepcopy(target_team["guns"]),
            "runtime_seconds": 0.0,
            "completed": False,
        })
        CONFIG["TEAM_ID"] = target_team["team_id"]
        CONFIG["FAIRY_ID"] = target_team["fairy_id"]
        CONFIG["FAIRY"] = copy.deepcopy(target_team.get("fairy"))
        CONFIG["GUNS"] = copy.deepcopy(target_team["guns"])
...
def gfam_request_index_for_emergency(client, manager=None):
    if manager is not None:
        resp = manager.bypass(client, reason="emergency_index")
    else:
    ...
    if isinstance(resp, dict) and "error" not in resp and "error_local" not in resp:
        update_fairy_cache_from_index_payload(resp, source="EPA Index/index")
        return resp
    return None
...
def gfam_refresh_dynamic_micro_limit_before_run(client):
    ...
    if not CONFIG.get("DYNAMIC_MICRO_BY_STORAGE", True):
        return True
    if _epa_index_mgr is not None:
        payload = _epa_index_mgr.get(reason="E2_pre_run_refresh")
    else:
        payload = gfam_request_index_for_emergency(client)
    if not payload:
...
def request_index_and_prepare_configs():
    ...
    global _epa_index_mgr
    if _epa_index_mgr is None and IndexCacheManager is not None:
        _epa_index_mgr = IndexCacheManager(client, ttl=60, label="EPA",
                                           shared_file=".gfam_index_cache.json")
    if _epa_index_mgr is not None:
        response = _epa_index_mgr.get(reason="E1_config_prepare")
    else:
    ...
    update_fairy_cache_from_index_payload(response, source="EPA 运行前 Index/index")
...
def epa_has_complete_runtime_selection() -> bool:
    """Runtime guard: EPA must not start from default mission constants only.

    MISSION_ID/START_SPOT/ROUTE have historical defaults, so they are not
    sufficient to prove the user selected a target.  The selected menu fields
    are the local state source for the current EPA target.
    """
    return bool(
        CONFIG.get("SELECTED_DIFFICULTY")
        and CONFIG.get("SELECTED_STAGE")
        and CONFIG.get("SELECTED_TARGET")
        and CONFIG.get("SELECTED_TARGET_LABEL")
    )
...
def gfam_verify_equip_locked_by_index(client, equip_uid):
    ...
    try:
        payload = {"time": int(time.time()), "furniture_data": False}
        if _epa_index_mgr is not None:
            idx = _epa_index_mgr.bypass(client, reason="equip_lock_verify")
        else:
...
def build_runtime_panel_lines():
    ...
    if epa_has_complete_runtime_selection():
        stage_label = "%s %s -> %s" % (
            CONFIG.get("SELECTED_DIFFICULTY"),
            CONFIG.get("SELECTED_STAGE"),
            CONFIG.get("SELECTED_TARGET_LABEL"),
        )
    else:
        stage_label = "未配置（已阻止运行）"
...
def print_run_summary():
    ...
    try:
        _mode_label = "练级五人模式" if CONFIG.get("MODE_NAME") == "team" else "打捞单人模式"
        _diff = CONFIG.get("SELECTED_DIFFICULTY") or ""
        _stage = CONFIG.get("SELECTED_STAGE") or ""
        _stage_label = "%s %s" % (_diff, _stage) if _diff and _stage else (_diff or _stage or "-")
        _target_label = CONFIG.get("SELECTED_TARGET_LABEL") or "未配置"
        _macro_count = int(RUN_STATS.get("current_macro") or 0)

        _stats = [
            {"label": "运行模式", "value": _mode_label},
            {"label": "关卡", "value": _stage_label},
            {"label": "目标", "value": _target_label},
            {"label": "完成轮数", "value": str(_macro_count)},
        ]

        _total_drops = 0
        if RUN_STATS.get("target_counts"):
            _drop_parts = []
            for _name, _item in RUN_STATS["target_counts"].items():
                _c = int(_item.get("count", 0))
                _total_drops += _c
                _drop_parts.append("%s×%d" % (_name, _c))
            _stats.append({"label": title, "value": "，".join(_drop_parts) if _drop_parts else "0"})
        _stats.append({"label": "目标掉落总数", "value": str(_total_drops)})
        ...
        try:
            ...
            if _gfam_os.path.exists(_factory_cache_path):
                ...
                _factory_data = {}
                for _fk, _flabel in [("doll", "人形"), ("equip", "装备")]:
...
def gfam_safe_abort_and_sync(client, reason="状态同步", attempts=2, sleep_seconds=1.2):
    ...
    try:
        payload = {"time": int(time.time()), "furniture_data": False}
        if _epa_index_mgr is not None:
            idx = _epa_index_mgr.bypass(client, reason="safe_abort_sync")
        else:
            idx = client.send_request(API_INDEX_INDEX, payload)
        if isinstance(idx, dict) and "error" not in idx and "error_local" not in idx:
            update_fairy_cache_from_index_payload(idx, source="EPA 状态同步 Index/index")
            try:
                if CONFIG.get("SELECTED_DIFFICULTY") == "夜战":
            ...
            if dropped is None:
                ...
                if CONFIG.get("SELECTED_DIFFICULTY") == "夜战":
                    ...
                    if CONFIG.get("ENABLE_GUN_EXCEPTION_SELF_REPAIR", True):
                        ...
                        if failure_limit_reached:
                            print("[EPA自修复] 连续失败达到上限且自修复已关闭，已自动终止当前运行。")
                            stop_macro_flag = True
                            stop_micro_flag = True
                            break
                        continue
...
def configure_capture_mode_after_login():
    """登录并抓到 UID/SIGN 后再选择编队/练级参数。"""
...
if __name__ == '__main__':
    ...
    if launched_from_gfam_main() and _gfam_auth_ready and CONFIG.get("INDEX_FETCH_READY", False):
        ...
        print("[*] 请先选择 epa_plus 运行配置；配置完成后再回游戏调整配队。")
        if configure_capture_mode_after_login():
            CONFIG["INDEX_FETCH_READY"] = False
            CONFIG["CONFIG_READY_FOR_INDEX"] = True
            print("")
            print("[*] epa_plus 运行配置已保存。")
            ...
            if cmd_prefix == '-a':
                ...
                if not CONFIG.get("INDEX_FETCH_READY", False) and not proxy_instance:
                    ...
                    if launched_from_gfam_main():
                        ...
                        print_server_menu()
                        server_cmd = input("GFL-EPA(服务器, 默认%s)> " % get_default_server_from_env()).strip()
                    ...
                    print("[*] 然后再次输入 -a，先选择编队模式和练级数量；选择完后再按提示调整配队。")
                    continue
                ...
                if CONFIG.get("INDEX_FETCH_READY", False):
                    ...
                    CONFIG["INDEX_FETCH_READY"] = False
                    if not configure_capture_mode_after_login():
                        continue
                    CONFIG["CONFIG_READY_FOR_INDEX"] = True
                    print("")
                    print("[*] 运行配置已保存。")
                ...
                if not epa_has_complete_runtime_selection():
                    print("[!] EPA 关卡/目标尚未完成配置，已阻止运行。")
                    print("[!] 请先完成：难度 -> 关卡 -> 目标；GUI 可使用“关卡并一键运行”。")
                    if MENU_STATE.get("selection_unlocked", False):
                        if MENU_STATE.get("difficulty") is None:
                            print_difficulty_menu()
                        elif MENU_STATE.get("stage") is None:
                            print_stage_menu(CONFIG.get("SELECTED_DIFFICULTY") or MENU_STATE.get("difficulty"))
                        else:
                            stage_data = get_stage_data(CONFIG.get("SELECTED_DIFFICULTY"), CONFIG.get("SELECTED_STAGE"))
                            if stage_data:
                                print_target_menu(CONFIG["SELECTED_DIFFICULTY"], CONFIG["SELECTED_STAGE"])
                    else:
                        print("[*] 当前尚未进入关卡选择流程；请先完成模式设置并输入 -a 获取/配置。")
                    continue
