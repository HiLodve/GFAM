...
def ticket_type_label():
    return "探查点数" if int(CONFIG.get("TICKET_TYPE", 2) or 2) == 1 else "四项资源"
...
def resolve_server_ticket_type_for_entry():
    ...
    返回 error:2 的情况。
    """
    mode = int(CONFIG.get("TICKET_TYPE", 2) or 2)
    if mode == 1:
        return 1
    ticket_val = None
    try:
...
def request_initial_index_and_cache(client):
    ...
    if check_step_error(resp, "Index/index"):
        runtime_log("[!] 运行前 Index/index 失败，已取消本次灰域自动彩蛋。", force=True)
        return False
...
def print_run_summary():
    ...
    if int(CONFIG.get("TICKET_TYPE", 2) or 2) == 2:
...
def halloween_farm_worker():
    ...
    try:
        ...
        if bool(CONFIG.get("ABORT_BEFORE_RUN", True)):
            ...
            abort_greyzone_missions(client, reason="运行前清理旧灰域关卡", quiet=True)
