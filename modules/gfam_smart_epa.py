...
_smart_index_mgr = None  # IndexCacheManager 实例，延迟初始化
...
def apply_epa_settings_from_file():
    """从 .gfam_epa_settings.json 加载 Smart EPA 设置并应用到 CONFIG。"""
    ...
    if target_module and target_module != "smart":
        return False

    # Smart EPA plan type
    plan_type = str(settings.get("smart_plan_type", "gun")).strip()
    if plan_type in ("gun", "equip"):
        CONFIG["SMART_EPA_TARGET_TYPE"] = plan_type
        applied.append("plan_type=" + plan_type)
    ...
    if applied:
        print("[*] 已从 EPA 设置文件加载 %d 项 Smart EPA 配置：" % len(applied))
        for item in applied:
            print("    %s" % item)
    return bool(applied)
...
def gfam_refresh_dynamic_micro_limit_before_run(client):
    ...
    if not CONFIG.get("DYNAMIC_MICRO_BY_STORAGE", True):
        return True
    if _smart_index_mgr is not None:
        payload = _smart_index_mgr.get(reason="S2_pre_run_refresh")
    else:
        payload = gfam_request_index_for_emergency(client)
    if not payload:
...
def request_index_and_prepare_configs():
    ...
    global _smart_index_mgr
    if _smart_index_mgr is None and IndexCacheManager is not None:
        _smart_index_mgr = IndexCacheManager(client, ttl=60, label="SmartEPA",
                                             shared_file=".gfam_index_cache.json")
    if _smart_index_mgr is not None:
        response = _smart_index_mgr.get(reason="S1_config_prepare")
    else:
...
def gfam_verify_equip_locked_by_index(client, equip_uid):
    ...
    try:
        payload = {"time": int(time.time()), "furniture_data": False}
        if _smart_index_mgr is not None:
            idx = _smart_index_mgr.bypass(client, reason="equip_lock_verify")
        else:
...
def gfam_safe_abort_and_sync(client, reason="状态同步", attempts=2, sleep_seconds=1.2):
    ...
    try:
        payload = {"time": int(time.time()), "furniture_data": False}
        if _smart_index_mgr is not None:
            idx = _smart_index_mgr.bypass(client, reason="safe_abort_sync")
        else:
            idx = client.send_request(API_INDEX_INDEX, payload)
