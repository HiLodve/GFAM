...
from gflzirc import (
    ...
    API_MISSION_ABORT, API_GUN_RETIRE, API_INDEX_INDEX, GUIDE_COURSE_11880
)
...
_f2p_index_mgr = None  # IndexCacheManager 实例，延迟初始化
...
def gfam_request_index_for_storage(client, *, bypass: bool = False):
    global _f2p_index_mgr
    if _f2p_index_mgr is None and IndexCacheManager is not None:
        _f2p_index_mgr = IndexCacheManager(client, ttl=60, label="f2p",
                                           shared_file=".gfam_index_cache.json")
    if _f2p_index_mgr is not None:
        if bypass:
            return _f2p_index_mgr.bypass(client, reason="storage_bypass")
        return _f2p_index_mgr.get(reason="storage_check")
    # 降级：原始逻辑
    try:
    ...
    try:
        ...
        update_fairy_cache_from_index_payload(resp, source="f2p Index/index")
    except Exception:
        pass
    return resp
...
def f2p_print_resource_summary(client):
    ...
    payload = gfam_request_index_for_storage(client, bypass=True)
    if payload is None:
    ...
    _summary = {
        "kind": "f2p", "title": F2P_DASHBOARD_STATS.get("mode_name") or "零元购",
        "elapsed_seconds": elapsed, "elapsed_text": f2p_format_seconds(elapsed),
        "macro": F2P_DASHBOARD_STATS.get("macro", 0),
        "total_runs": F2P_DASHBOARD_STATS.get("total_runs", 0),
        "total_drops": F2P_DASHBOARD_STATS.get("total_drops", 0),
        "retire_batches": F2P_DASHBOARD_STATS.get("retire_batches", 0),
        "retire_total": F2P_DASHBOARD_STATS.get("retire_total", 0),
        "stop_reason": F2P_DASHBOARD_STATS.get("stop_reason", ""),
        "resource_start": start_inv, "resource_end": end_inv,
        "resource_diff": diff, "resource_per_hour": per_hour,
    }
    try:
    ...
    try:
        ...
        pass
    try:
        with open(os.path.join(_gfam_root, ".gfam_f2p_summary.json"), "w", encoding="utf-8") as _f:
            json.dump(_summary, _f, ensure_ascii=False, indent=2)
    except Exception:
        pass
    print("[SUMMARY] %s 统计报告已生成。" % (F2P_DASHBOARD_STATS.get("mode_name") or "零元购"))
# === GFAM: 零元购资源统计结束 ===
...
def gfam_select_squad_from_index_payload(payload, reason="Index/index"):
    ...
    因此每次运行前从 Index/index 的 squad_with_user_info 自动选择一个可用重装小队 UID。
    """
    ...
    if env_uid > 0:
        for sq in squads:
            if sq["uid"] == env_uid:
                ...
                print("[重装] 已使用环境变量指定重装小队 UID=%s | squad_id=%s | Lv.%s | ammo=%s | mre=%s | life=%s" % (
    ...
    print("[重装] 已自动选择重装小队：UID=%s | squad_id=%s | Lv.%s | rank=%s | adv=%s | ammo=%s | mre=%s | life=%s" % (
...
def confirm_run_start():
    ...
    if index_payload is None:
        print("[重装] 无法请求 Index/index，因此无法读取重装小队 UID，已取消运行。")
        return False
    ...
    if not gfam_select_squad_from_index_payload(index_payload, reason="运行前 Index/index"):
        return False
    ...
    print("重装小队 UID：%s" % CONFIG.get("SQUAD_ID"))
    print("本次 Micro 上限：%s" % CONFIG.get("MISSIONS_PER_RETIRE"))
    print("Macro 上限：无固定上限，直到手动停止或进程受阻")
    print("说明：本模式使用 Index/index 中的 squad_with_user_info 自动选择重装小队。")
...
def gfam_pre_run_abort_once(client, reason="正式运行前状态清理"):
    ...
    if not CONFIG.get("ENABLE_PRE_RUN_ABORT", True):
        return False
    try:
        f2p_log("[状态清理] %s：尝试 abortMission mission_id=11880。" % reason)
...
def _farm_worker_impl():
    ...
    f2p_start_dashboard("零元购 f2p", 11880)
    f2p_render_dashboard(force=True)
    macro = 1
    consecutive_failures = 0
            ...
            if dropped is None:
                ...
                if CONFIG.get("ENABLE_GUN_EXCEPTION_SELF_REPAIR", True):
                    ...
                    if gfam_try_exception_self_repair(client, reason="零元购 Micro Error Before Retry"):
                        retry_drop = farm_mission_11880(client, CONFIG["SQUAD_ID"])
...
def print_menu():
    ...
    if CONFIG.get("GFAM_AUTH_READY"):
        ...
        print("提示：运行前会通过 Index/index 自动选择重装小队。")
    else:
