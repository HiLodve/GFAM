...
_f2p_pr_index_mgr = None  # IndexCacheManager 实例，延迟初始化
...
def gfam_request_index_for_storage(client, *, bypass: bool = False):
    global _f2p_pr_index_mgr
    if _f2p_pr_index_mgr is None and IndexCacheManager is not None:
        _f2p_pr_index_mgr = IndexCacheManager(client, ttl=60, label="f2p_pr",
                                              shared_file=".gfam_index_cache.json")
    if _f2p_pr_index_mgr is not None:
        if bypass:
            return _f2p_pr_index_mgr.bypass(client, reason="storage_bypass")
        return _f2p_pr_index_mgr.get(reason="storage_check")
    # 降级：原始逻辑
    try:
...
def f2p_print_resource_summary(client):
    ...
    _summary = {
        "kind": "f2p", "title": F2P_DASHBOARD_STATS.get("mode_name") or "零元购PR",
    ...
    try:
        ...
        pass
    try:
        with open(os.path.join(_gfam_root, ".gfam_f2p_pr_summary.json"), "w", encoding="utf-8") as _f:
            json.dump(_summary, _f, ensure_ascii=False, indent=2)
    except Exception:
        pass
    print("[SUMMARY] %s 统计报告已生成。" % (F2P_DASHBOARD_STATS.get("mode_name") or "零元购PR"))
# === GFAM: 零元购资源统计结束 ===
