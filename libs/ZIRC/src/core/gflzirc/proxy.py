...
def set_windows_proxy(enable: bool, proxy_addr="127.0.0.1:8080"):
    ...
    try:
        ...
        if enable:
            _register_proxy_cleanup()
        refresh_windows_proxy()
        return True
    except Exception:
        return False

_proxy_cleanup_registered = False

def _register_proxy_cleanup():
    """Register an atexit handler to ensure Windows proxy is disabled on exit."""
    global _proxy_cleanup_registered
    if _proxy_cleanup_registered:
        return
    _proxy_cleanup_registered = True
    _atexit.register(lambda: set_windows_proxy(False))
...
class GFLProxy:
    def __init__(self, port: int, static_key: str, on_traffic_callback=None):
        ...
        self._client_semaphore = threading.Semaphore(50)  # Max 50 concurrent client threads
    ...
    def _handle_client_with_sem(self, client_sock):
        try:
        ...
        try:
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    if self._client_semaphore.acquire(blocking=False):
                        t = threading.Thread(target=self._handle_client_with_sem, args=(client_sock,))
                        t.daemon = True
                        t.start()
                    else:
                        client_sock.close()
